...
public partial class BonPreparationListViewModel : BaseViewModel
{
    ...
    {
        BtnNew = _locale.T("Btn_NewBonPreparation");
        BtnPdf = _locale.T("Btn_Pdf");
        UpdateBtnFilterDateText();
        WmFilterPayee = _locale.T("Bp_FilterPayee");
        LblPayeeFilterAll = _locale.T("Bp_FilterAll");
        LblPayeeFilterUnpaid = _locale.T("Bp_Unpaid");
        LblPayeeFilterPaid = _locale.T("Bp_Paid");
        ...
        ColHeaderPayee = _locale.T("BpList_ColPayee");
    ...
    public ObservableCollection<BonPreparationListRow> Items { get; } = [];
    [ObservableProperty] private BonPreparationListRow? _selected;
    ...
    private async Task LoadPageAsync(CancellationToken ct, bool resetPage = false)
    {
        ...
        try
        {
            ...
            var q = db.BonsPreparation.AsNoTracking().Include(f => f.Lignes).AsQueryable();
            ...
            var selId = Selected?.BonPreparation.Id;
            Items.Clear();
            foreach (var f in list)
                Items.Add(BonPreparationListRow.Create(f, noms.GetValueOrDefault(f.ClientId) ?? string.Empty, devise, _locale));
            Pagination.TotalCount = total;
            if (selId is { } id)
                Selected = Items.FirstOrDefault(x => x.BonPreparation.Id == id);
        }
        finally
        {
            IsBusy = false;
    ...
    private void OpenSelected()
    {
        ...
        var vm = _sp.GetRequiredService<BonPreparationEditViewModel>();
        vm.Load(Selected.BonPreparation.Id);
        _workspace.Open(vm);
    }

    [RelayCommand]
    private async Task DeleteBonPreparationAsync(BonPreparationListRow? row, CancellationToken cancellationToken)
    {
        if (row == null) return;
        var item = row.BonPreparation;

        if (!await _dialog.ConfirmAsync(_locale.T("Bp_Title"), _locale.Tf("Bp_ConfirmDelete", item.Numero), cancellationToken))
            return;
        ...
        try
        {
            ...
            if (Selected?.BonPreparation.Id == item.Id)
                Selected = null;
            Items.Remove(row);
    ...
    private async Task ExportPdfAsync(CancellationToken cancellationToken)
    {
        ...
        {
            await using var db = await _dbFactory.CreateDbContextAsync(cancellationToken);
            var f = await db.BonsPreparation.Include(x => x.Lignes).Include(x => x.Paiements).FirstAsync(x => x.Id == Selected.BonPreparation.Id, cancellationToken);
            var client = await db.Tiers.AsNoTracking().FirstAsync(t => t.Id == f.ClientId, cancellationToken);
            var bytes = await _pdf.BuildBonPreparationPdfAsync(f, DocumentPartyPdfInfo.FromTiers(client), cancellationToken);
