...
public partial class BonDechargeEditViewModel : BaseViewModel
{
    ...
    public BonDechargeEditViewModel(
    ...
    public bool CanDelete => BonDechargeId.HasValue;

    partial void OnBonDechargeIdChanged(int? value)
    {
    ...
    public async Task LoadAsync(int? id, CancellationToken cancellationToken = default)
    {
        ...
        var users = await db.Users.AsNoTracking().Where(u => u.Actif).OrderBy(u => u.FullName)
        ...
        if (id == null)
        {
            ...
            Title = _locale.T("BDH_NewTitle");
            RefreshTotals();
            return;
        }

        var b = await db.BonsDecharge.Include(x => x.Lignes).FirstAsync(x => x.Id == id, cancellationToken);
        Numero = b.Numero;
        ...
        Title = _locale.Tf("BDH_EditTitle", Numero);
        RefreshTotals();
    }
    ...
    private async Task SaveAsync(CancellationToken cancellationToken)
    {
        ...
        {
            await _dialog.ShowErrorAsync(_locale.T("BDH_Title"), _locale.T("BCH_ErrDepot"), cancellationToken);
            return;
        }

        if (!Lignes.Any(l => l.ProduitId > 0 && l.Quantite > 0))
        {
            await _dialog.ShowErrorAsync(_locale.T("BDH_Title"), _locale.T("BCH_ErrLines"), cancellationToken);
            return;
        }
        ...
        try
        {
            await using var db = await _dbFactory.CreateDbContextAsync(cancellationToken);
            BonDecharge entity;
            if (BonDechargeId == null)
            {
                var num = await _numbers.NextBonDechargeAsync(cancellationToken);
                entity = new BonDecharge
                {
                    Numero = num,
                ...
                {
                    entity.Lignes.Add(new BonDechargeLigne
                    {
                        ProduitId = l.ProduitId,
                        Designation = l.Designation,
                        Quantite = l.Quantite,
                ...
                db.BonsDecharge.Add(entity);
                await db.SaveChangesAsync(cancellationToken);
                BonDechargeId = entity.Id;
            }
            else
            {
                entity = await db.BonsDecharge.Include(b => b.Lignes)
    ...
    {
        if (BonDechargeId is not { } id) return;
    ...
    private void Back()
    {
        var list = _sp.GetRequiredService<BonDechargeListViewModel>();
