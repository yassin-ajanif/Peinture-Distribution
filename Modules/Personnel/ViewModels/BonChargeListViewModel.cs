...
public partial class BonChargeListViewModel : BaseViewModel
{
    ...
    public ObservableCollection<BonChargeListRow> Items { get; } = [];
    ...
    [ObservableProperty] private BonChargeListRow? _selected;

    private void RefreshUi()
    {
        Title = _locale.T("BCH_Title");
        BtnNew = _locale.T("Btn_New");
        BtnFilterDate = _locale.T("Btn_FilterDate");
        SearchWatermark = _locale.T("BCH_SearchWm");
    ...
    private async Task LoadPageAsync(CancellationToken ct, bool resetPage = false)
    {
        ...
        try
        {
            ...
            var q = db.BonsCharge.AsNoTracking().AsQueryable();
            ...
            {
                Items.Add(new BonChargeListRow
                {
                    Doc = d,
    ...
    private void NewDoc()
    {
        var vm = _sp.GetRequiredService<BonChargeEditViewModel>();
        vm.Load(null);
        _workspace.Open(vm);
    }
